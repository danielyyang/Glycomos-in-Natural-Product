        ...
        for _ in range(len(ring_atoms)-2):
            ...
            for n in curr.GetNeighbors():
                if n.GetIdx() in ring_atoms and n.GetIdx() != prev:
                    ...
                    break
            if not found: 
                print(f"Broke path early at {curr.GetIdx()}, path so far: {path}")
                break
...
trace_signature(clean_mol, mapped_ring)
